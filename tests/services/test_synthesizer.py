...
class TestSynthesizer:
    ...
    def sample_recommended_series(self):
        ...
        return [
            ...
            RecommendedSerie(
                title="My Hero Academia", id_series="series-uuid-3", responce_IA="Histoire inspirante de héros modernes"
            ),
        ]
    ...
    def test_generate_global_response_openai(
        self, mock_openai_class, mock_settings, synthesizer, sample_recommended_series, sample_user_profile
    ):
        ...
        mock_completion.choices[0].message.content = (
            "Voici mes recommandations personnalisées pour vous basées sur votre profil."
        )
        ...
        with patch.dict("os.environ", {"USE_AZURE_OPENAI": "false"}):
            ...
            result = synthesizer.generate_global_response(
                recommended_series=sample_recommended_series, user_profile=sample_user_profile
            )
    ...
    def test_generate_global_response_azure(
        self, mock_azure_openai_class, mock_settings, synthesizer, sample_recommended_series, sample_user_profile
    ):
    ...
    def test_generate_global_response_no_series(
        self, mock_openai_class, mock_settings, synthesizer, sample_user_profile
    ):
        ...
        with patch.dict("os.environ", {"USE_AZURE_OPENAI": "false"}):
            # Exécution
            result = synthesizer.generate_global_response(recommended_series=[], user_profile=sample_user_profile)
    ...
    def test_generate_global_response_with_collection(
        ...
        with patch.dict("os.environ", {"USE_AZURE_OPENAI": "false"}):
            # Exécution
            result = synthesizer.generate_global_response(
                recommended_series=sample_recommended_series, user_profile=sample_user_profile_with_collection
            )
